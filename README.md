(https://user-images.githubusercontent.com/446285/92606997-31e0c480-f279-11ea-9fde-eb8fe04201ba.png) # Elastic stack (ELK) on Docker
...
> features](#disable-paid-features) section to opt out of this be-haviour.
...
1. [How to Configure](#configuration)
   ...
   * [scale out the Elasticsearch cluster](#scale-out-the-elasticsearch-cluster)
   * [re-execute the setup](#re-execute-the-setup)
   * [reset a password programmatically](#reset-a-password-programmatically)
1. [Extensibility](#extensibility)
   * [add plugins](#add-plugins)
   * [enable the provided extensions](#enable-the-provided-extensions)
1. [JVM tuning](#jvm-tuning)
   * [specify the amount of memory used by a service](#specify-the-amount-of-memory-used-by-a-service)
   * [enable a remote JMX connection to a service](#enable-a-remote-jmx-connection-to-a-service)
...
1. Replace usernames and passwords in configuration files
    ...
    Its value isn't used by any core component, but [extensions](#enable-the-provided-extensions) use it to
    connect to Elasticsearch.

    > [!NOTE]
    > In case you don't plan on using any of the provided [extensions](#enable-the-provided-extensions), or
...
* user: *elastic*
* password: *\<your generated elastic pass-word>*
...
This repository remains synchronized with the most recent iteration of the Elastic stack. The main branch reflects the ongoing major version (8.x).

To employ an alternative version of the fundamental Elastic components, adjust the version identifier within the .env file. 
When upgrading a current stack, ensure to reconstruct all container images by executing the docker-compose build command.
...
Please refer to the following documentation page for more details about configure Elasticsearch inside Docker
...
Please refer to the following documentation page for more details about configure Kibana inside Docker
...
Please refer to the following documentation page for more details about configure Logstash inside Docker
...
Follow the instructions from the Wiki: [Scaling out Elasticsearch](https://github.com/pvnakum7/Elastic-Stack-ELK-on-Docker/wiki/Elastic-Search-Cluster.md)

### re-execute the setup

To rerun the setup container and reinitialize all users for whom a password was specified in the **'.env'** file, simply restart the setup Compose service by bringing it "up" again.
...
In case you encounter any obstacles preventing you from utilizing Kibana to alter user passwords (including [built-in users][builtin-users]), an alternative method is available using the Elasticsearch API. This approach allows you to accomplish the same outcome.

In the example below, use command to reset the password of the `elastic` user (notice "/user/elastic" in the URL):
...
1. Incorporate a `RUN` directive into the relevant `Dockerfile` (for example, `RUN logstash-plugin install logstash-filter-json`).
2. Integrate the plugin's code configuration into the service setup (for instance, Logstash input/output).
3. Execute the `docker-compose build` command to reconstruct the images.

### enable the provided extensions

Inside the extensions directory, you'll find several additional functionalities. These extensions offer enhancements beyond the standard Elastic stack, allowing users to incorporate extra integrations.

Each extension includes its documentation within its respective subdirectory. Some may necessitate manual adjustments to the default ELK configuration to function optimally.
...
The initialization scripts for Elasticsearch and Logstash are configured to append additional JVM options based on the value of an environment variable. This flexibility enables users to fine-tune the memory allocation for each component as needed.
...
For setups with limited memory resources (like Docker Desktop for Mac, which typically offers only 2 GB by default), the default Heap Size allocation is restricted in the docker-compose.yml file to 512 MB for Elasticsearch and 256 MB for Logstash. If you wish to customize the JVM configuration beyond these defaults, you can modify the corresponding environment variable(s) directly within the docker-compose.yml file.
...
* Logstash starts with a fixed JVM Heap Size of 1 or 2 GB.

### enable a remote JMX connection to a service

Regarding the Java Heap memory (as mentioned earlier), you have the ability to define JVM options to activate JMX and link the JMX port on the Docker host.

Modify the {ES,LS}_JAVA_OPTS environment variable with the provided details below (I've set the JMX service to be mapped on port 18080, which you can customize). Remember to adjust the -Djava.rmi.server.hostname option with the IP address of your Docker host (substitute **DOCKER_HOST_IP**):
